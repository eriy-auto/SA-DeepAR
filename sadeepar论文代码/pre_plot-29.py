...
x1 = [i for i in range(len(sadp_data) - 1)]

fig, ax = plt.subplots(
    nrows=1, ncols=1, figsize=(10, 6), sharey="row", sharex="col"
)
ax.set_ylabel("power(KW)")
ax.plot(x1, sadp_data[:-1, 5], lw=2, label="Test data", c="b")  

ax.plot(
    x1, sadp_data[1:, 0], lw=2, c="C1", label="sadeepar_Pre"
)
ax.plot(
    x1, enbpi_data[:-2, 0], lw=2, c="C2", label="enbpi_Pre"
)
ax.plot(
             x1, sarima_data[:-3, 0]+200, lw=2, c="c", label="sarima_Pre"
         )
ax.fill_between(
    x1,
    sadp_data[1:, 1],
    ...
    color="C1",
    alpha=0.2,
    label="sadeepar_Prediction intervals",
)
ax.fill_between(
    x1,
    enbpi_data[:-2, 1],
    ...
    color="C2",
    alpha=0.2,
    label="enbpi_Prediction intervals",
)
...
ax.fill_between(
            ...
            color="c",
            alpha=0.2,
            label="sarima_Prediction intervals",
        )

ax.set_title('Power pre')
...
sa_dp_rmse = np.sqrt((np.sum((sadp_data[3151:3501, 5] - sadp_data[3151:3501, 0]) ** 2) / n))
sa_dp_mape = np.sum(np.abs((sadp_data[3151:3501, 5] - sadp_data[3151:3501, 0]) / sadp_data[3151:3501, 5])) / n
sa_dp_picp = 0
sm = 0
for i in range(3151, 3151+m):

    if sadp_data[i, 1] <= sadp_data[i, 5] <= sadp_data[i, 2]:
        sm += 1
sa_dp_picp = sm/m
pinaw = np.sum(sadp_data[3151:3501, 2] - sadp_data[3151:3501, 1])
...
print('sadp_mape:', sa_dp_mape)

enbpi_rmse = np.sqrt((np.sum((sadp_data[3151:3501, 5] - enbpi_data[3148:3498, 0]) ** 2) / n))
enbpi_mape = np.sum(np.abs((sadp_data[3151:3501, 5] - enbpi_data[3148:3498, 0]) / sadp_data[3151:3501, 5])) / n
enbpi_picp = 0
sm = 0
for i in range(3148, 3148+m):

    if enbpi_data[i, 1] <= sadp_data[i, 5] <= enbpi_data[i, 2]:
        sm += 1
enbpi_picp = sm/m
pinaw = np.sum(enbpi_data[3148:3498, 2] - enbpi_data[3148:3498, 1])
enbpi_pinaw = pinaw/(R*m)
print('enbpi_picp:', enbpi_picp, 'enbpi_pinaw:', enbpi_pinaw)
print('enbpi_rmse:', enbpi_rmse)
print('enbpi_mape:', enbpi_mape)

sarima_rmse = np.sqrt((np.sum((sadp_data[3151:3501, 5] - sarima_data[3147:3497, 0]) ** 2) / n))
sarima_mape = np.sum(np.abs((sadp_data[3151:3501, 5] - sarima_data[3147:3497, 0]) / sadp_data[3151:3501, 5])) / n
sarima_picp = 0
sm = 0
for i in range(3147, 3147+m):

    if sarima_data[i, 2] <= sadp_data[i, 5] <= sarima_data[i, 1]:
        sm += 1
sarima_picp = sm/m
pinaw = np.sum(sarima_data[3147:3497, 1] - sarima_data[3147:3497, 2])
...
sadp_sigma = np.sqrt(sum([((sadp_data[i, 1] - sadp_data[i, 0]) / 18) ** 2 for i in range(1, len(sadp_data))]))
sadp_samples_normal = np.random.normal(sadp_mu, sadp_sigma, 100000)

enbpi_mu = ep_data_power
enbpi_sigma = np.sqrt(sum([((enbpi_data[i, 1] - enbpi_data[i, 0]) / 18) ** 2 for i in range(0, len(enbpi_data) - 2)]))
enbpi_samples_normal = np.random.normal(enbpi_mu, enbpi_sigma, 100000)

sari_mu = sari_data_power
sari_sigma = np.sqrt(sum([((sarima_data[i, 1] - sarima_data[i, 0]) / 18) ** 2 for i in range(0, len(sarima_data) - 3)]))
